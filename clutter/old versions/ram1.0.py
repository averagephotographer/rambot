...
class item():
    ...
    def __str__(self):
        return "{0.type}: {0.content}".format(self)
...
class rawResponse(response):
    def messageIn(self,message):
        if self.inn == None: return True
        return message.content.lower() == self.inn.lower() 
...
class lockedResponse(response):
    def messageIn(self,message):
        ...
        if text[-5:] == lock():
            ...
            return response.messageIn(self,message)
        return False
...
#class personalResponse(rawResponse):
#    def __init__(self,textIn,content)
#        self.useKey = False
#        response.__init__(self,textIn,content,label)
#
#    def messageIn(self,message):
#        if message.user == 
#            if self.useKey:
#                return response.messageIn(message)
#            return rawResponse.messageIn(message)
...
#    out = item("text",process.extractOne(problem,contents)[0])
    ...
    #global responses
    #add response for webwork solve
    return item("text",best + '\n\n' + out)

def webworkSolve(equations,answers,new=None):
    for i in equations:
        x=0
        i = list(i)
        while x<len(i):
